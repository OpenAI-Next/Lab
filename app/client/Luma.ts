...
export interface LumaExtendTaskRequest {
    aspect_ratio: "16:9";                   // 固定为16:9, luma官方暂时不支持更改，请暂时固定传16：9
    expand_prompt: boolean;                 // 是否优化扩展提示词
    image_end_url?: any;                    // 结束帧图片，关键帧
    user_prompt: string;                    // 提示词
    video_url: any;                         // 需要扩展的视频 URL，可以不是 luma 生成的视频，任意第三方视频或者 luma 的视频都可
}

export type AccountType = "relax" | "vip";
...
export class LumaApi {
    ...
    async lumaExtendTask(request: LumaExtendTaskRequest, accountType: AccountType, signal?: AbortSignal, timeoutMs: number = REQUEST_TIMEOUT_MS) {
            ...
            try {
                const res = await fetch(this.path(accountType, LumaEndpoint.EXTEND), {
                    ...getRequestOptions(this.apiKey, request),
                    signal: abortSignal
                });
    ...
    async queryLumaTask(request: { id: string }, accountType: AccountType, signal?: AbortSignal, timeoutMs: number = REQUEST_TIMEOUT_MS) {
