...
export class LumaApi {
  ...
  path(accountType: AccountType, endpoint: LumaEndpoint) {
    return [api2ProviderBaseUrl.Pika, endpoint]
      .join("/")
      .replace("{{luma_account_type}}", AccountType2Path[accountType]);
  }

  async generateLumaTask(
    request: LumaGenerationTaskRequest,
  ...
  async lumaExtendTask(
    request: LumaExtendTaskRequest,
    accountType: AccountType,
    signal?: AbortSignal,
    timeoutMs: number = REQUEST_TIMEOUT_MS,
  ) {
