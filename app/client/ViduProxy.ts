...
export class ViduAPI {
  ...
  availableTaskTypes(
    prompts: ViduTaskGenerationRequest["input"]["prompts"],
    isUpscale = false,
  ) {
    if (isUpscale) {
  ...
  randomTextPrompt(): string {
    return randomTextPrompt[
      Math.floor(Math.random() * randomTextPrompt.length)
    ];
  }

  async generateViduTask(
    request: ViduTaskGenerationRequest,
    signal?: AbortSignal,
    ...
    try {
      ...
      console.error(
        "[ViduProxy] failed to make a vidu generate-task request",
        e,
      );
      throw e;
    }
  ...
  async submitUpscaleTask(
    request: ViduUpscaleTaskRequest,
    signal?: AbortSignal,
    timeoutMs: number = 10000,
  ) {
    const controller = new AbortController();
    ...
    try {
      ...
      console.error(
        "[ViduProxy] failed to make a vidu upscale-task request",
        e,
      );
      throw e;
    }
  ...
  async getViduTask(
    id: ViduTaskGenerationResponse["id"],
    signal?: AbortSignal,
    timeoutMs: number = 10000,
  ) {
    const controller = new AbortController();
    ...
    try {
      const res = await fetch(
        this.path(ViduEndpoint.TASK_GET).replace("{{id}}", id),
        {
