...
const ImagineForm = (props: {
    ...
    const selectedBotType = (ProForm.useWatch("botType", props.form) || "MID_JOURNEY") as MidjourneyImagineTaskRequestPayload["botType"];
    ...
    return (
        <ProForm<MidjourneyImagineTaskConfigType>
            ...
            onFinish={async (values) => {
                await handleSubmit(values, "IMAGINE", props.api, props.updateResponse, props.updateError, setSubmitting, setAbortController);
            }}
            submitter={{
                render: (submitterProps) => {
                    return <RenderSubmitter
                        ...
                        getValues={() => JSON.stringify(props.api.getImagineTaskPayload(props.form.getFieldsValue()) || "", null, 2)}
                    />
                }
            }}
        ...
        >
            ...
            <ProFormTextArea
                ...
                fieldProps={{autoSize: {minRows: isCustomParam ? 5 : 3, maxRows: 8}}}
                rules={[{required: true}]}
            />

            {!isCustomParam && inputPrompt.length > 30 && <ProForm.Item>
                <Button
                    block
                    type="dashed"
            ...
            {!isCustomParam && <>
                ...
                <ProForm.Group title={"Parameters"}>
                    ...
                    <ProFormDigit
                        ...
                        min={0}
                        max={3000}
                        tooltip={"Explore unconventional aesthetics with the experimental --weird or --w parameter. This parameter introduces quirky and offbeat qualities to your generated images, resulting in unique and unexpected outcomes."}
                    ...
                    <ProFormSelect
                        ...
                        rules={[{type: "enum", enum: differentModelsDetail[selectedBotType].styleOptions}]}
                    />
                    ...
                    <ProFormTextArea
                        ...
                        tooltip={"The No parameter tells the Midjourney Bot what not to include in your image. --no accepts multiple words separated with commas."}
                    ...
                    <ProFormUploadButton
                        ...
                        tooltip={"You can use images as style references in your prompt to influence the style or aesthetic of images you want Midjourney to make.\n"}
                        accept={"image/*"}
                        max={2}
...
const BlendForm = (props: {
    ...
    return (
        <ProForm<MidjourneyBlendTaskConfigType>
            ...
            onFinish={async (values) => {
                await handleSubmit(values, "BLEND", props.api, props.updateResponse, props.updateError, setSubmitting, setAbortController);
            }}
            submitter={{
                render: (submitterProps) => {
                    return <RenderSubmitter
...
const DescribeForm = (props: {
    ...
    return (
        <ProForm<MidjourneyDescribeTaskRequestPayload>
            ...
            form={props.form}
            onFinish={async (values) => {
                await handleSubmit(values, "DESCRIBE", props.api, props.updateResponse, props.updateError, setSubmitting, setAbortController);
            }}
            submitter={{
                render: (submitterProps) => {
                    return <RenderSubmitter
...
const ShortenForm = (props: {
    ...
    return (
        <ProForm<MidjourneyShortenTaskRequestPayload>
            ...
            form={props.form}
            onFinish={async (values) => {
                await handleSubmit(values, "SHORTEN", props.api, props.updateResponse, props.updateError, setSubmitting, setAbortController);
            }}
            submitter={{
                render: (submitterProps) => {
                    return <RenderSubmitter
...
const ActionForm = (props: {
    ...
    return (
        <ProForm<MidjourneyDoActionQueryType>
            ...
            form={props.form}
            onFinish={async (values) => {
                await handleSubmit(values, "ACTION", props.api, props.updateResponse, props.updateError, setSubmitting, setAbortController);
            }}
            submitter={{
                render: (submitterProps) => {
                    return <RenderSubmitter
...
const MidjourneyTasksRenderer = (props: {
    ...
    const handleRefresh = async (index: number) => {
        ...
        try {
            if (props.tasks) {
                const res = await props.api.fetchTask({taskId: props.tasks[index].id});
                const status = res.status;
                const resJson = await res.json()
                        ...
                        columns={[
                            ...
                            },
                            {
                                ...
                                render: () => [
                                    ...
                                    <a
                                        key="delete"
                                        onClick={() => props.onUpdated(task, true)}
                                    >
                                        Delete
                                    </a>,
                                ],
...
export function MidjourneyPage() {
    ...
    const onScrollToBottom = () => {
        requestAnimationFrame(() => {
            dataAreaRef.current?.scrollTo({top: dataAreaRef.current.scrollHeight + 1000, behavior: "smooth"});
        });
    };
    ...
    const updateError = (error: any) => {
        if (error !== null) {
            message.error({content: "Error occurred.", key: "midjourney-error", duration: 2}).then()
            onScrollToBottom()
        }
        setErrorData(error)
    }
